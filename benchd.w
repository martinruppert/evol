...
@*1 $x_0^2+x_1^2$.
@<die Benchmarkfunktionen@>=
int x0qpx1q(double x[], long iz, double f[], object*hobj, object**objmge){
  int i=0;
  f[0]=0.0;
...
@ @<Optimiere $x_0^2+x_1^2$@>=
  ...
  for(is=0;is<20;is++){
    x[0]=1.0; s[0]=0.001;
    x[1]=1.0; s[1]=0.001;
    izaehl=devol(&x0qpx1q,x,s,2,f,0,10000);
  ...
  printf("%d %d %d #x0^2+x1^2 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);

@*1 Die Funktion von Powell.
@<die Benchmarkfunktionen@>=
int powell(double x[], long iz, double f[], object*hobj, object**objmge){
  double xh1,xh2,xh3,xh4;
  xh1 = x[0] + 10.0*x[1];
  xh1 = xh1 * xh1;
  xh2 = x[2] - x[3];
  xh2 = 5.0 * xh2 * xh2;
...
@ @<powell-text@>=
    izaehl=devol(powell,x,s,4,f,0,20000);
    if(izaehl>=0)iges+=izaehl;
    else         iges-=izaehl;
    if(izaehl<imin)imin=izaehl;
    if(izaehl>imax)imax=izaehl;
...
#define POWELLS(a,b,c,d) x[0]=a;x[1]=b;x[2]=c;x[3]=d; 
  ...
  printf("%d %d %d #powell:3,-1,0,1 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);
  ...
  printf("%d %d %d #powell:-1,-10,0,1 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);
  ...
  printf("%d %d %d #powell:1,1,1,1 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);
  ...
  printf("%d %d %d #powell:10,10,10,10 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);

@*1 Die Rosenbrock'sche Klippenfunktion.
@<die Benchmarkfunktionen@>=
int rosenklippe(double x[], long iz, double f[], object*hobj, object**objmge){
  double xh;
  xh=(x[0]-3.0)/100.0;
  f[0]=xh*xh;
  xh=x[0]-x[1];
...
@ @<Optimiere die Rosenbrock-Klippenfunktion@>=
  ...
  for(is=0;is<20;is++){
    x[0]=0.0;x[1]=-1.0;
    izaehl=devol(rosenklippe,x,s,2,f,0,2000000);
    if(izaehl>=0)iges+=izaehl;
    else         iges-=izaehl;
    if(izaehl<imin)imin=izaehl;
    if(izaehl>imax)imax=izaehl;
  ...
  printf("%d %d %d #rosenklippe:0,-1 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);

  for(i=0;i<2;i++)s[i]=1e-3;
  imin=INT_MAX; imax=0; iq=0;
  for(is=0;is<20;is++){
    x[0]=-1.57;x[1]=-1.73;
    izaehl=devol(rosenklippe,x,s,2,f,0,20000);
    if(izaehl>=0)iges+=izaehl;
    else         iges-=izaehl;
    if(izaehl<imin)imin=izaehl;
    if(izaehl>imax)imax=izaehl;
  ...
  printf("%d %d %d #rosenklippe:-1.57,-1.73 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);
  ...
  printf("%d %d %d #rosenklippe:0,0 Mittelwerte der lsg min avg max\n",imin,iq/is,imax);

@*1 Die Kreiselmeier-Funktion.
@<die Benchmarkfunktionen@>=
int kreiselfun(double x[], long iz, double f[], object*hobj, object**objmge){
  int i;
  double xh,z[9];
  xh=x[1];
  xh-=x[0]*sin(x[0]);
...
@ @<Optimiere die Kreiselmeier-Funktion@>=
  ...
  for(is=0;is<20;is++){
    ...
    izaehl=devol(kreiselfun,x,s,9,f,0,200000);
    if(izaehl>=0)iges+=izaehl;
    else         iges-=izaehl;
    if(izaehl<imin)imin=izaehl;
    if(izaehl>imax)imax=izaehl;
  ...
  printf("%d %d %d #kreiselfun: Mittelwerte der lsg min avg max\n",imin,iq/is,imax);

...
%%%Aktivieren, wenn devol selbstaendig dokumentiert wird
...
@<DefinitionModules@>=
double pt1(double y, double u, double t);
int tst_file(char*s);
...
@<DefinitionModules@>=
int guetefunktion(double x[], long izaehl, double f[],object*hobj,object**objmge);
...
long devol(@t\1@>@/
    int    guetefunktion(double x[], long izaehl,double f[],object*hobj,object**objmge),@/
    double x[],   /* x[0..npar-1]   EinAusgabe, Start- und Ergebnisvektor  */
    double s[],   /* s[0..npar-1]   Einausgabe, Streuungsvektor */
    int    npar, /*                Eingabe                     */
    double f[],   /* f[0..nres-1]   R\"uckgabe, Zielf.-vektor   */
...
Gleitkommazahl (z.B. 1e-32) f\"ur 32Bit~IEEE~Double Zahlen vorgegeben.
...
struct strategie_p strategie={@t\1@>@/
      30,     /*int aktiveobjekte*/
       3,     /*int besteobjekte*/
     150,     /*int maxalter*/
       0.25,  /*double sollerfolg*/
       0.25,  /*double erfolg*/
       1.0,   /*double erfolgsbewertung*/
    5000,     /*double tmess*/
       1e-30, /*double eamin*/
       1e-7,  /*double ebmin*/
       1e-1,  /*double ebmax*/
       3.0,   /*double gradinfo*/
       1.0,   /*double startrwdf*/
     1000,    /*int zwischenausg*/
       10,     /*int verbessausg*/
...
@*1 Realisierende Funktion |int devol(...)|.
...
@<Generierung und Initialisierung der Startpopulation@>=
  ...
  for(objzahl=0;objzahl<aktiveobjekte;objzahl++){
    ...
    objmge[objzahl]->streuungen=(double*)calloc(npar,sizeof(double));
    objmge[objzahl]->parameter=(double*)calloc(npar,sizeof(double));
    if(objzahl==0){
      for(i=0;i<npar;i++){
    ...
    objmge[objzahl]->guetevektor=(double*)calloc(nres+1,sizeof(double));
  }
...
if(strategie.verbessausg>0){@t\1@>@;
  ...
  PVERB(@+for(i=0;i<=nres;i++)printf(" %lg",bobj->guetevektor[i]);@+)@;
  PVERB(for(i=0;i<=nres;i++))@;
  PVERB(printf(" %lg",objmge[indvekt[aktiveobjekte-2]]->guetevektor[i]);)@;
  PVERB(printf(" %lg",strategie.erfolg);)@;
...
if(izaehl%strategie.zwischenausg==0){@t\1@>@;
  ...
  printf("%ld",izaehl);
  for(i=0;i<=nres;i++){
    printf(" %lg",objmge[indvekt[0]]->guetevektor[i]);
  }
  for(i=0;i<=nres;i++){
    printf(" %lg",objmge[indvekt[aktiveobjekte-1]]->guetevektor[i]);
  } 
  printf(" %lg #mevol_verlauf.ausgabe iz fb[] fs[] erfolg\n",
          strategie.erfolg);
...
void allocobj(object**hobj,int n,int m){
  ...
  (*hobj)->streuungen=(double*)calloc(n,sizeof(double));
  (*hobj)->parameter=(double*)calloc(n,sizeof(double));
  (*hobj)->guetevektor=(double*)calloc(m+1,sizeof(double));
}
...
int readobjekt(object*hobj, long*izaehl, 
  ...
  if(infile == NULL){
    ...
    for(i=0;i<=hobj->restriktionsanzahl;i++){
      fscanf(infile,"%lg",&(hobj->guetevektor[i]));
    }
